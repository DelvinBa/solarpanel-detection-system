...
| **Orchestration**   | **Apache Airflow** | - Industry-standard tool for workflow orchestration <br> - Scalable, supports distributed execution <br> - Large community and extensive plugin support <br> - Integrates well with MinIO, Python, and ML pipelines                                                                                                      | Prefect, Dagster, Kubeflow     | - **Prefect** is simpler but lacks some advanced DAG capabilities <br> - **Dagster** is great for data pipelines but less mature for ML workflows <br> - **Kubeflow** is powerful but complex to set up and maintain                                                                             |
...
| **Database**        | **PostgreSQL**     | - Perfect integration with Airflow (Airflow's native metadata DB) <br> - PostGIS extension for geospatial data (key for property location analysis) <br> - Strong data integrity with ACID compliance <br> - Open-source with no licensing costs <br> - JSON support for flexible schema <br> - Scalable for logging/analytics | SQL Server, MongoDB, TimescaleDB | - **SQL Server** has licensing costs and higher resource demands <br> - **MongoDB** has weaker integration with Airflow and less mature spatial capabilities <br> - **TimescaleDB** is specialized for time-series data, which only partially fits our use case                                  |
| **Data Versioning** | **DVC**            | - Integrates with Git to handle large files and data versioning <br> - Ensures reproducible ML experiments by tracking data changes <br> - Lightweight, open-source, and well-documented <br> - Works seamlessly with existing code and cloud storage (MinIO)                                                           | Git LFS, Git-annex             | - **Git LFS** can become expensive or inconvenient for large datasets <br> - **Git-annex** is powerful but less straightforward to integrate with ML workflows <br> - Other solutions often lack the specialized ML and data pipeline focus that DVC provides                                  |
| **Experiment Tracking** | **MLflow**    | - Popular open-source tool for experiment logging and model registry <br> - Easy to integrate with Python-based ML pipelines <br> - Tracks metrics, parameters, and artifacts in a centralized manner <br> - Supports model packaging and deployment (model registry)                                                 | Weights & Biases, Neptune.ai, Azure ML | - **Weights & Biases** and **Neptune.ai** are SaaS offerings (potential cost & vendor lock-in) <br> - **Azure ML** is powerful but can be overkill for smaller teams and requires more Azure-specific setup <br> - MLflow strikes a balance between simplicity, flexibility, and cost-effectiveness |
