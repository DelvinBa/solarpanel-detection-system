---
title: "Technical/Functional Design"
author:
  - Navid Gharapanjeh
  - Delvin Bacho
date: "2025-02-07"
...
### Goal 2: BAG/House ID Detection  
**In order to** link detected solar panels with housing data for better energy label predictions,  
**As** the Project Lead,  
**I want** to establish a relation between detected solar panels and **BAG/House ID**, ensuring at least 85% successful matches.  
...
**We want** to develop a **complete MLOps pipeline** that covers **data ingestion, preprocessing, model training, deployment, and monitoring**.
...
## 1️ Performance
- **Training Efficiency**  
  - The training pipeline should complete within **≤3 hours for 50 epochs** when using GPU.  
  - **Batch size and augmentation strategies** must optimize GPU utilization.  

- **Inference Speed**  
  - The model should perform **real-time inference in ≤10ms per image (on GPU)**.  
  - **Batch processing** should handle **1000+ images in <10 minutes**.  

- **Model Accuracy**  
  - **mAP@50 ≥ 90% for 'High' class**, **mAP@50-95 ≥ 50% for all categories**.  
  - Precision-Recall balance must be maintained at **>85% recall for critical classes**.  

---

## 2️ Scalability
- **Inference Scaling**  
  - The system should support **scaling up to 1000 requests per second (RPS)**.  
  - Use **GPU autoscaling (AWS EKS, EC2 G5, or ECS Fargate with GPU)** for large workloads.  

- **Storage Scalability**  
  - The system should handle **>10TB of image data**, using **MinIO with auto-tiering to AWS S3 Glacier**.  

---

## 3️ Security & Compliance
- **Access Control**  
  - Implement **Role-Based Access Control (RBAC)** for model access using AWS IAM.  
  - **Airflow DAG execution should be restricted to authorized users** via IAM roles.  

- **Compliance**  
  - The system must comply with **GDPR for user-generated image data storage and processing**.  
  - **Model access logs must be maintained for 180 days** to enable forensic audits.  

---

## 5️ Maintainability & Observability
- **Monitoring & Logging**  
  - **Model training metrics (loss, mAP, IoU) should be logged in MLflow**.  
  - **System logs should be stored in AWS CloudWatch or ELK Stack**.  

- **Alerting & Issue Tracking**  
  - Implement **AWS SNS/Slack alerts** for model degradation (mAP drop ≥ 10%).  
  - Use **Prometheus + Grafana** for monitoring MinIO, inference latency, and Airflow DAGs.  

- **CI/CD & Model Deployment**  
  - New models must be **automatically deployed if mAP ≥ baseline threshold**.  
  - **Rollback to previous `best.pt` should be possible within 5 minutes** in case of failures.  
